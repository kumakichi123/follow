...
export default async function EstimateViewPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const planOptions = [
    {
      key: 'matsu' as const,
      title: estimate.matsu_label ?? '????',
      subtitle: 'Premium',
      amount: estimate.matsu_amount ?? estimate.amount ?? 0,
      description: estimate.matsu_description ?? '???????????????????????????',
      perks: ['????????', '????????', '??????????'],
      recommended: false
    },
    {
      key: 'take' as const,
      title: estimate.take_label ?? '????',
      subtitle: 'Standard',
      amount: estimate.take_amount ?? estimate.amount ?? 0,
      description: estimate.take_description ?? '???????????????????????????',
      perks: ['????????', '????????', '????1?'],
      recommended: true
    },
    {
      key: 'ume' as const,
      title: estimate.ume_label ?? '????',
      subtitle: 'Light',
      amount: estimate.ume_amount ?? estimate.amount ?? 0,
      description: estimate.ume_description ?? '???????????????????????',
      perks: ['??????', '???????', '?????????'],
      recommended: false
    }
  ]
